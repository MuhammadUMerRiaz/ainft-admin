...
const Artist = (props) => {
    ...
    useEffect(() => {
        if(artistId){
            setArtist({description: props?.artist?.description, name: props?.artist?.name, _id: props?.artist?._id})
            setSubmissionType(2)
        }
    }, [props.artist])
                ...
                loader ?
                    ...
                    <Container>
                        <Form>
                            ...
                            <Form.Group>
                                <label>Image {submissionType === 1 && <span className="text-danger">*</span>}</label>
